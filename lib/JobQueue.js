...
class JobQueue {
  constructor(options = {}) {
    ...
    this._blockNotifyUrls = [];
    if (this._options.blockNotifyUrls) {
      for (const blockedUrlPattern of this._options.blockNotifyUrls) {
        if (blockedUrlPattern instanceof RegExp) {
          this._blockNotifyUrls.push(blockedUrlPattern);
        } else {
          let re = blockedUrlPattern;
          if (!(re.startsWith('^') && re.endsWith('$'))) {
            re = '^' + re.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&') + '$';
          }
          this._blockNotifyUrls.push(new RegExp(re, 'g'));
        }
      }
    }
    this._jobClasses = {
  ...
  _shouldNotify(url) {
    for (const re of this._blockNotifyUrls) {
      if (url.match(re)) {
        return false;
      }
    }
  ...
  async subscribe(options) {
    ...
    const newJobCheckInterval = options.newJobCheckInterval ||
      ...
      2;
    let teamRefill = options.teamRefill;
    if (typeof teamRefill === 'undefined') {
      teamRefill = true;
    }
  ...
  async pushItems(items, queueName = 'mail-main') {
    try {
      ...
      await this._queue.insert(jobs);
      return true;
    } catch (err) {
      this._logger.error(
        'ERROR: unexpected error occurred while queueing the items',
        err
      );
    }
  }
  ...
  async notify(target, type, content, options = {}, guid) {
    try {
      ...
      this._logger.info('--- JobQueue notify request ready ---', {
        url,
        target,
        guid
      });

      if (this._shouldNotify(url)) {
        const request = {
          url,
          headers,
          data: {
        ...
        this._logger.info(`notification to ${url} is blocked by options`);
      }
    } catch (err) {
