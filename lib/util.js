...
const getDkim = async function(dir, domain, config, defaultDkim = false) {
  ...
  const selector = config.dkimSelector || 'main';
...
const fetchRoutingInfo = async function(environment, transport) {
  const headers = transport.headers || {};
  let routingUrl = urlJoin(environment.baseUrl, environment.routingUri);
  const directUrl = headers[DIRECT_DYNAMIC_ROUTING_URL_HEADERNAME];
...
const headerListToObject = function(headers) {
  const result = {};
  for (const header of (headers || [])) {
    if (!Object.prototype.hasOwnProperty.call(result, header[0])) {
      result[header[0]] = [];
    }
    const values = result[header[0]];
...
const headerLinesToObject = function(headerLines) {
  return headersToObject(new Headers(headerLines));
};
...
const adjustEnvelope = function(envelope) {
  ...
  const replyTo = extractAddress(headers['reply-to']);
...
const emailToTransport = function(email) {
  const message = new MailComposer(email).compile();
  const envelope = message.getEnvelope();
  const headersStr = message.buildHeaders();
  const headers = new Headers(headersStr);
  envelope.headers = headers;
  const transport = envelopeToTransport(envelope);
  const rfc822Headers = getRfc822Headers(email);
  if (rfc822Headers) {
    transport['rfc822_headers'] = rfc822Headers;
  }
  return transport;
};

const getRfc822Headers = function(mail) {
  const attachments = mail.attachments || [];
  for (const attachment of attachments) {
    if (attachment.contentType === 'text/rfc822-headers') {
      const headersBuffer = attachment.content;
      const headers = headersToObject(new Headers(headersBuffer));
      return headers;
    }
  }
};

const extractAttachments = function(mail) {
  const attachments = [];
...
const getJsonSafeHeaders = function(mail) {
  return [...mail.headers];
};

const getJsonSafeEmail = function(mail) {
  const attachments = extractAttachments(mail);
  const headers = getJsonSafeHeaders(mail);
  return {
    ...mail,
    headers,
    attachments
...
const getRfc822Message = async function(mail) {
  const attachments = mail.attachments || [];
  for (const attachment of attachments) {
    if (attachment.contentType === 'message/rfc822') {
      const rawMail = attachment.content;
      const messageMail = await simpleParser(rawMail, {Iconv});
      return getJsonSafeEmail(messageMail);
    }
  }
};
