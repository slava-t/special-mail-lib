...
const emailToTransport = function(email) {
  const emailAddresses = getAddressesFromEmail(email);
  const emailKeys = new Set(Object.keys(email));
  let envelope = email.envelope || {};
  if (!emailKeys.has('envelope')) {
    if (emailAddresses.from.length > 0) {
      envelope.from = emailAddresses.from[0].address;
    }
    if (emailAddresses.to.length > 0) {
      envelope.to = emailAddresses.to;
    }
    if (emailAddresses.cc.length > 0) {
      envelope.cc = emailAddresses.cc;
    }
    if (emailAddresses.bcc.length > 0) {
      envelope.bcc = emailAddresses.bcc;
    }
  }
  ...
  return envelopeToTransport(envelope);
};
