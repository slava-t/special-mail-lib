...
module.exports =  class DomainNameVerifier {
  ...
  async _verifyDkim(environment, domain) {
    ...
    if (!Array.isArray(dkimTxtRecords) || dkimTxtRecords.length === 0) {
      return {
        errors: [createError(
          'DkimDnsResolveError',
          dkim.domain,
  ...
  async _createSetSpfStep(spfInclude, mailServers, domain) {
    let failed = false;
    const spfPrefix = 'v=spf1 ';
    const spfIncludeStatement = `include:${spfInclude}`;
    let dnsRecords = [];
    try {
    ...
    if (failed || !spfRecord) {
      const dnsNewValue = `v=spf1 ${spfIncludeStatement} ~all`;
      return {
        step: 'SpfSetDnsRecord',
        message: `Set a DNS TXT record for the ${domain} domain to include ` +
          `${spfInclude}. An example for a valid SPF value would be ` +
          `'${dnsNewValue}'.`
      };
    }
    const spfRemaining = spfRecord.substring(spfPrefix.length);
    const suggestedValue = `${spfPrefix}${spfIncludeStatement} ${spfRemaining}`;
    return {
  ...
  async _verifySpf(environment, domain) {
    const {mailServers, spfInclude} = this._environments[environment];
    ...
    if (errors.length > 0) {
      const spfStep = await this._createSetSpfStep(
        spfInclude,
        mailServers,
        domain
      );
